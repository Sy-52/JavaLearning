...
// >> TODO 函数式编程是把函数作为基本运算单元。函数可作为变量、可接收函数、可返回函数。把支持函数式编程的编码风格称为Lambda表达式。
...
//        myList.forEach(inst::processStringNonStatic);
        ...
        myMap.put("k2","v2");
        myMap.put("k3","v3");
        // TODO Map的forEach和List的forEach又不一样了，见源码。
...
//        System.out.println(str);
        // TODO 可以发现，action.accept直接跳到了下面这句。其实这中间Java在后台一顿操作帮我们生成了一个接口的匿名实现类，并在其中调用了processString()。
